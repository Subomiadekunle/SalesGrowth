...
def get_user_details():
    st.title("🏠 Welcome to SalesGrowth")
    st.markdown("Upload your own sales data or try out our app using a sample sales file!")

    # 🔽 Sample file download
    sample_file_path = "sample_sales.csv"  # Make sure this file exists in the root folder
    if os.path.exists(sample_file_path):
        ...
        st.error("⚠️ Sample sales file not found. Please make sure it's in your project folder.")

    # 👤 User name + role input
    if "user_name" not in st.session_state:
        user_name = st.text_input("👤 What is your name?")
        if user_name:
            st.session_state.user_name = user_name
    ...
    if "user_name" in st.session_state and "data_type" not in st.session_state:
        data_type = st.radio("📊 Is this data for a company or personal use?", ["Company", "Personal"])
        if data_type:
            st.session_state.data_type = data_type.lower()
            if data_type == "Company":
                ...
                if company_name:
                    st.session_state.company_name = company_name
                    st.success(f"Great! Now upload the {company_name} sales sheet.")
            else:
                st.success("Awesome! Now upload your personal sales data.")
